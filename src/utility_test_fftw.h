...
bool RunFFTWTest(const std::vector<complex_t>& out, const SignalInfo& info, int64_t sparsity, int rank = 1) {
    ...
    auto result = GetSignalFromMap(RecursiveSparseFFT(x, info, sparsity, rank), info);
