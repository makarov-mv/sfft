...
def get_args():
    ...
    parser.add_argument("--last_logn", action='store', default=8, type=int, help='logarithm of the last value of n to test')
    ...
    parser.add_argument("--signal_type", choices=['random', 'comb', 'combined', 'twocomb'], default='combined', help='choose signal type to run experiments on')
...
title = '{}D {}, sparsity={}, {} projection'.format(
        args.dimensions, "comb-mixture" if args.signal_type == 'twocomb' else args.signal_type + " signal", args.sparsity, 'with' if args.use_projection_recovery else 'no'
    )
...
make_graph('{}-dimensional-{}-{}{}'.format(
        args.dimensions, args.signal_type, args.sparsity, '-Proj' if args.use_projection_recovery else '') + args.output, title, p, algs, plot_names, results, args.samples, args.plot_scale, args.extra_data)
